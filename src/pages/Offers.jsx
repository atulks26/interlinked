...
const WhatWeOffer = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setCurrentIndex(
                (prevIndex) =>
                    (prevIndex + 1) % (offers.length - slidesToShow + 1)
            );
        }, 5000);
    ...
    return (
        <div className="w-full flex flex-col items-center justify-center my-10 py-10">
            <h2 className="text-3xl font-bold mb-6 m-auto">What We Offer</h2>
            <div className="relative w-full max-w-5xl bg-white shadow-lg rounded-lg overflow-hidden">
                ...
                <div className="w-full flex justify-center items-center my-10">
                    <div className="w-full h-full flex space-x-4 px-6">
                        {offers
                            .slice(currentIndex, currentIndex + slidesToShow)
                            .map((offer, idx) => (
                                <div
                                    key={idx}
                                    className="flex-1 text-center p-4 bg-gray-100 rounded-lg"
                                >
                                    <img
                                        src={offer.image}
                                        alt={offer.title}
